...
Функция СтруктураВСтроку(Структ, Главный = Неопределено)
	...
	Для каждого Элемент Из Структ Цикл
		...
		ИначеЕсли ЭлементКлюч = "Главный" Тогда
			ЭлементЗначение = Число(ЭлементЗначение) + 1;
		ИначеЕсли ЭлементКлюч = "Соседний" И ЭлементЗначение = Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Функция ВыгрузитьУзел(Узел, Главный = Неопределено)
	...
	Если Узел.Свойство("Дочерний") Тогда
		Результат = Результат + ВыгрузитьУзел(Узел.Дочерний, Узел.КодУзла);
	КонецЕсли;

	Если Узел.Свойство("Соседний") Тогда
		Результат = Результат + ВыгрузитьУзел(Узел.Соседний, Узел.КодУзла);
	КонецЕсли;
